...
module MarkdownIt
  ...
  class Ruler 
    ...
    # * ```
    #------------------------------------------------------------------------------
    def before(beforeName, ruleName, fn, alt : Array(String)=[""])
    ...
    # * ```
    #------------------------------------------------------------------------------
    def after(afterName, ruleName, fn, alt : Array(String)=[""])
    ...
    # * ```
    #------------------------------------------------------------------------------
    def push(ruleName, fn, alt : Array(String)=[""])
      unless alt.includes?("")
        alt = [""] + alt
      end
    ...
    def enable(list, ignoreInvalid = false)
      ...
      list.each do |name|
        ...
        if idx < 0
          ...
          raise("Rules manager: invalid rule name #{name}")
        end
    ...
    def getRules(chainName)
      __compile__ if @__cache__.empty?
