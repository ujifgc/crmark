module LinkifyIt
  ...
  SRC_TLDS = TLDS_2CH_SRC_RE + '|' + TLDS_DEFAULT + '|' + SRC_XN

  EMAIL_FUZZY      = Regex.new(TPL_EMAIL_FUZZY.gsub("%TLDS%", SRC_TLDS), Regex::Options::IGNORE_CASE)
  LINK_FUZZY       = Regex.new(TPL_LINK_FUZZY.gsub("%TLDS%", SRC_TLDS), Regex::Options::IGNORE_CASE)
  LINK_NO_IP_FUZZY = Regex.new(TPL_LINK_NO_IP_FUZZY.gsub("%TLDS%", SRC_TLDS), Regex::Options::IGNORE_CASE)
  HOST_FUZZY_TEST  = Regex.new(TPL_HOST_FUZZY_TEST.gsub("%TLDS%", SRC_TLDS), Regex::Options::IGNORE_CASE)

  SCHEMA_TEST   = Regex.new("(^|(?!_)(?:[><\uff5c]|" + SRC_Z_P_CC + "))(" + SCHEMAS_LIST + ")", Regex::Options::IGNORE_CASE)
